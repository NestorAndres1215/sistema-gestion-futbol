...
export default function useEstadioEstadisticas() {
  ...
  const fetchEstadisticas = async () => {
    try {
      ...
      const [
        totalEstadiosRes, promedioCapacidadRes, totalPaisesRes,
        paisesMasRes, paisesMenosRes, ciudadesMasRes,
        ciudadesMenosRes, mayorCapacidadRes, menorCapacidadRes,
        estadiosAntiguosRes, estadiosNuevosRes, distribucionEstadoRes, tiposCespedRes
      ] = await Promise.all([
        getTotalEstadios(), getPromedioCapacidad(), getTotalPaises(),
        getPaisesMasEstadios(5), getPaisesMenosEstadios(5), getCiudadesMasEstadios(5),
        getCiudadesMenosEstadios(5), getMayorCapacidad(5), getMenorCapacidad(5),
        getEstadiosMasAntiguos(5), getEstadiosMasNuevos(5), getDistribucionEstado(), getTiposCesped()
      ]);

      const totalEstadiosData = totalEstadiosRes?.data ?? totalEstadiosRes;
      const promedioCapacidadData = promedioCapacidadRes?.data ?? promedioCapacidadRes;
      const totalPaisesData = totalPaisesRes?.data ?? totalPaisesRes;

      setTotalEstadios(totalEstadiosData.total || 0);
      setPromedioCapacidad(promedioCapacidadData.promedio || 0);
      setTotalPaises(totalPaisesData.total || 0);
      setPaisesMasEstadios(paisesMasRes?.data ?? paisesMasRes ?? []);
      setPaisesMenosEstadios(paisesMenosRes?.data ?? paisesMenosRes ?? []);
      setCiudadesMasEstadios(ciudadesMasRes?.data ?? ciudadesMasRes ?? []);
      setCiudadesMenosEstadios(ciudadesMenosRes?.data ?? ciudadesMenosRes ?? []);
      setEstadiosMayorCapacidad(mayorCapacidadRes?.data ?? mayorCapacidadRes ?? []);
      setEstadiosMenorCapacidad(menorCapacidadRes?.data ?? menorCapacidadRes ?? []);
      setEstadiosMasAntiguos(estadiosAntiguosRes?.data ?? estadiosAntiguosRes ?? []);
      setEstadiosMasNuevos(estadiosNuevosRes?.data ?? estadiosNuevosRes ?? []);
      setDistribucionEstado(distribucionEstadoRes?.data ?? distribucionEstadoRes ?? []);
      setTiposCesped(tiposCespedRes?.data ?? tiposCespedRes ?? []);
  ...
  return {
    ...
    paisesMasEstadios, paisesMenosEstadios, ciudadesMasEstadios, ciudadesMenosEstadios,
    estadiosMayorCapacidad, estadiosMenorCapacidad, estadiosMasAntiguos, estadiosMasNuevos,
    distribucionEstado, tiposCesped, estadoItems
  };
}